...
class Resolver {
  ...
  scanFiles(cwd) {
    ...
    entryFiles.forEach(entry => {
      ...
      const definition = this.loadFile(definitionFile);
      ...
      if (!this.values[value]) {
        this.values[value] = !properties.length
          ? (hasDependencies && new Injector(definition, this.loadFile(providerFile)) || definition)
          : {};
      }
      ...
      while (properties.length) {
        ...
        if (!target[propName]) {
          if (hasDependencies && !properties.length) {
            target[propName] = new Injector(definition, this.loadFile(providerFile));
          } else {
