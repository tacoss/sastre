...
class Resolver {
  ...
  static scanFiles(cwd, cb) {
    ...
    Object.keys(resolverInfo.values).forEach(prop => {
      let definition = resolverInfo.values[prop];

      if (typeof cb === 'function' && typeof definition !== 'function') {
        if (Injector.supports(resolverInfo.values[prop])) {
          definition = definition.valueOf();
        }
