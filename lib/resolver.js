...
class Resolver {
  ...
  static scanFiles(cwd, cb) {
    ...
    entryFiles.forEach(entry => {
      ...
      const isInjectable = typeof definition === 'function' && definition.length === 1;

      if (!resolverInfo.values[value]) {
        resolverInfo.values[value] = !properties.length
          ? ((isInjectable && new Injector(definition, Resolver.useFile(providerFile))) || definition)
          : {};
      }
      ...
      while (properties.length) {
        ...
        if (!target[propName]) {
          if (isInjectable && !properties.length) {
            target[propName] = new Injector(definition, Resolver.useFile(providerFile));
          } else {
  ...
  get(name) {
    try {
      ...
      throw new Error(`${e.message} (${path.basename(this._directory)})`);
    }
  }
}
