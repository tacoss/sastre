...
class Resolver {
  constructor(cwd, hooks) {
    this._decorator = Resolver.get(hooks);

    const container = Resolver.scanFiles(cwd, this._decorator.before);

    this._registry = container.registry;
    this._values = container.values;
  ...
  static get(hooks) {
    ...
    return ['before', 'after'].reduce((prev, cur) => {
      prev[cur] = (...args) => hooks[cur] && hooks[cur](...args);

      return prev;
    }, {});
  }

  static use(container, hooks) {
    ...
    return (name, definition) => {
      ...
      Object.keys(definition.dependencies).forEach(key => {
        ...
        const factoryValue = definition.dependencies[key].call(container);
  ...
  static bind(resolver, definition) {
    ...
    const deps = Object.keys(definition.dependencies);
    ...
    deps.forEach(key => {
      ...
      const factory = definition.dependencies[key].bind(resolver);
