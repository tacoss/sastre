...
class Injector {
  constructor(definition, injectables) {
    if (typeof definition !== 'function') {
      throw new ScopeError(`Cannot inject non-callable values, given ${inspect(definition)}`);
    }

    if (!injectables || Array.isArray(injectables) || typeof injectables !== 'object') {
      throw new ScopeError(`Invalid injectables, given ${inspect(injectables)}`);
    }
  ...
  static assign(target, extensions) {
    if (['object', 'function'].indexOf(typeof target) === -1 || Array.isArray(target)) {
      throw new ScopeError(`Target is not an object, given ${inspect(target)}`);
    }

    if (target[RESOLVED_STATE]) {
      throw new ScopeError('Cannot assign to locked values');
    }
  ...
  static bind(resolver, definition, afterCallback) {
    ...
    Object.getOwnPropertyNames(deps).forEach(key => {
      ...
      if (keys.indexOf(propName) === -1) {
        throw new ScopeError(`Missing '${propName}' dependency`);
      }

      if (!resolver.values[propName]) {
        throw new ScopeError(`Value '${propName}' is not defined`);
      }
      ...
      if (typeof method !== 'function') {
        throw new ScopeError(`Invalid resolver, given ${inspect(method)}`);
      }
    ...
    keys.forEach(key => {
      Object.defineProperty(proxy, key, {
        ...
        get: () => {
          if (!values[key]) {
            throw new ScopeError(`Missing '${key}' provider`);
          }
        ...
        get: () => {
          if (typeof values[key] === 'undefined') {
            if (typeof deps[key] !== 'function') {
              throw new ScopeError(`Invalid resolver, given ${inspect(deps[key])}`);
            }
